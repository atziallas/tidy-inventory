...
import {
  getDesignatedLocation,
  getDesignatedSublocation,
} from "../../common/reducers/designatedLocations";
...
function* doAction() {
  ...
  if (
    ...
    const messages = [
      { message: "File downloaded", show: false, tags: ["success"] },
    ];
...
export function* designate() {
  const json = yield call(
    Api.designate,
    yield select(getSelections),
    yield select(getDesignatedLocation),
